import { db, films, people, planets, species } from "../models/db.ts";
...
export const getCharacters = async (ctx: any, next: any) => {
  ...
  try {
    const data: any = await people.find({}, { noCursorTimeout: false })
      .toArray();
    ctx.response.body = {
      status: true,
      data: data,
...
export const getOneCharacter = async (ctx: any, next: any) => {
  try {
    ...
    console.log(id);
    const data: any = await people.findOne({ _id: new Bson.ObjectId(id) }, {
...
export const getFilms = async (ctx: any, next: any) => {
  try {
    const data: any = await films.find({}, { noCursorTimeout: false })
      .toArray();
    ctx.response.body = {
      status: true,
      data: data,
...
export const getPlanets = async (ctx: any, next: any) => {
  try {
    const data: any = await planets.find({}, { noCursorTimeout: false })
      .toArray();
    ctx.response.body = {
      status: true,
      data: data,
...
export const getSpecies = async (ctx: any, next: any) => {
  try {
    const data: any = await species.find({}, { noCursorTimeout: false })
      .toArray();
    ctx.response.body = {
      status: true,
      data: data,
...
export const checkApiKey = async (ctx: any) => {
  ...
  if (ctx.request.headers.has("api_key")) {
    api_key = ctx.request.headers.get("api_key");
  } else api_key = "";

  const selectedUser: any = await Users.findOne({ api_key }, {
    noCursorTimeout: false,
  });
